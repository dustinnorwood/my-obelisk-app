...
import           Backend.Database              (VayconDb (vayconFollows, vayconUsers), QueryError,
                                                  vayconDb, maybeRow, singleRow)
import           Backend.Database.Users.Follow (Follow, FollowT (Follow))
import qualified Backend.Database.Users.Follow as Follow
import           Backend.Database.Users.User   (PrimaryKey (UserId), User, UserId, UserT (User))
import qualified Backend.Database.Users.User   as User
import           Backend.Validation            (ValidationErrors, requiredText)
import           Common.Api.Profiles.Profile   (Profile (Profile))
import           Common.Api.User.Update        (UpdateUser (UpdateUser))
import qualified Common.Api.User.Update        as UpdateUser
import           Common.Api.Users.Credentials  (Credentials)
import qualified Common.Api.Users.Credentials  as Credentials
import           Common.Api.Users.Registrant   (Registrant (Registrant))
import qualified Common.Api.Users.Registrant   as Registrant
insertUser
  :: Registrant -> PgInsertReturning User
insertUser reg
  = insertReturning
    (vayconUsers vayconDb)
    (insertExpressions
      [ User
          { User.id = default_
...
selectUserBy
  ...
  -> PgQExpr s a
  -> Q Postgres VayconDb s (UserT (PgQExpr s))
selectUserBy f a = do
...
follows
  :: UserT (PgQExpr s)
  -> Q Postgres VayconDb s (FollowT (Nullable (PgQExpr s)))
follows author =
  leftJoin_
    (all_ (vayconFollows vayconDb))
...
selectProfiles
  :: Maybe UserId
  -> Q Postgres VayconDb s (ProfileRow s)
